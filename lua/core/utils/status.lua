nvim.status = { hl = {}, init = {}, provider = {}, condition = {}, component = {}, utils = {}, env = {} }

nvim.status.env.modes = {
      ["n"] = { "NORMAL", "normal" },
      ["no"] = { "OP", "normal" },
      ["nov"] = { "OP", "normal" },
      ["noV"] = { "OP", "normal" },
...
nvim.status.env.icon_highlights = {
  file_icon = {
...
function nvim.status.hl.lualine_mode(mode, fallback)
...
function nvim.status.hl.mode() return { bg = nvim.status.hl.mode_bg() } end
...
function nvim.status.hl.mode_bg() return nvim.status.env.modes[vim.fn.mode()][2] end
...
function nvim.status.hl.filetype_color(self)
...
function nvim.status.hl.get_attributes(name, include_bg)
  local hl = nvim.status.env.attributes[name] or {}
  hl.fg = name .. "_fg"
...
function nvim.status.hl.file_icon(name)
  return function(self)
    local hl_enabled = nvim.status.env.icon_highlights.file_icon[name]
    if type(hl_enabled) == "function" then hl_enabled = hl_enabled(self) end
    if hl_enabled then return nvim.status.hl.filetype_color(self) end
  end
end
...
function nvim.status.init.breadcrumbs(opts)
  opts = nvim.default_tbl(opts, {
    separator = " > ",
    icon = { enabled = true, hl = nvim.status.env.icon_highlights.breadcrumbs },
  ...
  return function(self)
    ...
    for i, d in ipairs(data) do
      local pos = nvim.status.utils.encode_pos(d.lnum, d.col, self.winnr)
      local child = {
        ...
        on_click = {
          ...
          callback = function(_, minwid)
            local lnum, col, winnr = nvim.status.utils.decode_pos(minwid)
    ...
    if opts.padding.left > 0 then -- add left padding
      table.insert(children, 1, { provider = nvim.pad_string(" ", { left = opts.padding.left - 1 }) })
    end
    if opts.padding.right > 0 then -- add right padding
      table.insert(children, { provider = nvim.pad_string(" ", { right = opts.padding.right - 1 }) })
    end
...
function nvim.status.init.update_events(opts)
  return function(self)
...
function nvim.status.provider.fill() return "%=" end
...
function nvim.status.provider.tabnr()
...
function nvim.status.provider.spell(opts)
  opts = nvim.default_tbl(opts, { str = "", icon = { kind = "Spellcheck" }, show_empty = true })
  return function() return nvim.status.utils.stylize(vim.wo.spell and opts.str, opts) end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.paste(opts)
  opts = nvim.default_tbl(opts, { str = "", icon = { kind = "Paste" }, show_empty = true })
  return function() return nvim.status.utils.stylize(vim.opt.paste:get() and opts.str, opts) end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.macro_recording(opts)
  opts = nvim.default_tbl(opts, { prefix = "@" })
  return function()
    local register = vim.fn.reg_recording()
    if register ~= "" then register = opts.prefix .. register end
    return nvim.status.utils.stylize(register, opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.search_count(opts)
  ...
  return function()
    ...
    if search_ok and type(search) == "table" and search.total then
      return nvim.status.utils.stylize(
        string.format(
          "%s%d/%s%d",
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.mode_text(opts)
  local max_length =
      math.max(unpack(vim.tbl_map(function(str) return #str[1] end, vim.tbl_values(nvim.status.env.modes))))
  return function()
    local text = nvim.status.env.modes[vim.fn.mode()][1]
    ...
    return nvim.status.utils.stylize(text, opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.percentage(opts)
  opts = nvim.default_tbl(opts, { fixed_width = false, edge_text = true })
  return function()
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.ruler(opts)
  opts = nvim.default_tbl(opts, { pad_ruler = { line = 0, char = 0 } })
  local padding_str = string.format("%%%dd:%%%dd", opts.pad_ruler.line, opts.pad_ruler.char)
  return function()
    local line = vim.fn.line "."
    local char = vim.fn.virtcol "."
    return nvim.status.utils.stylize(string.format(padding_str, line, char), opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.scrollbar(opts)
  local sbar = { "▁", "▂", "▃", "▄", "▅", "▆", "▇", "█" }
  return function()
    local curr_line = vim.api.nvim_win_get_cursor(0)[1]
    local lines = vim.api.nvim_buf_line_count(0)
    local i = math.floor((curr_line - 1) / lines * #sbar) + 1
    return nvim.status.utils.stylize(string.rep(sbar[i], 2), opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.close_button(opts)
  opts = nvim.default_tbl(opts, { kind = "BufferClose" })
  return nvim.status.utils.stylize(nvim.get_icon(opts.kind), opts)
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.filetype(opts)
  return function(self)
    local buffer = vim.bo[self and self.bufnr or 0]
    return nvim.status.utils.stylize(string.lower(buffer.filetype), opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.filename(opts)
  opts = nvim.default_tbl(
    opts,
  ...
  return function(self)
    ...
    return nvim.status.utils.stylize((filename == "" and opts.fallback or filename), opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.unique_path(opts)
  opts = nvim.default_tbl(opts, {
  ...
  return function(self)
    ...
    for _, value in ipairs(vim.g.heirline_bufferline and vim.t.bufs or nvim.status.utils.get_valid_buffers()) do
    ...
    return nvim.status.utils.stylize(
      ...
      and string.sub(unique_path, 1, opts.max_length - 2) .. nvim.get_icon "Ellipsis" .. "/"
      ) or unique_path,
      opts
    )
  end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.file_modified(opts)
  opts = nvim.default_tbl(opts, { str = "", icon = { kind = "FileModified" }, show_empty = true })
  return function(self)
    return nvim.status.utils.stylize(
      nvim.status.condition.file_modified((self or {}).bufnr) and opts.str,
      opts
    )
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.file_read_only(opts)
  opts = nvim.default_tbl(opts, { str = "", icon = { kind = "FileReadOnly" }, show_empty = true })
  return function(self)
    return nvim.status.utils.stylize(
      nvim.status.condition.file_read_only((self or {}).bufnr) and opts.str,
      opts
    )
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.file_icon(opts)
  return function(self)
    local devicons_avail, devicons = pcall(require, "nvim-web-devicons")
    if not devicons_avail then return "" end
    ...
    return nvim.status.utils.stylize(ft_icon, opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.git_branch(opts)
  return function(self) return nvim.status.utils.stylize(vim.b[self and self.bufnr or 0].gitsigns_head or "", opts) end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.git_diff(opts)
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.diagnostics(opts)
  ...
  return function(self)
    ...
    return nvim.status.utils.stylize(count ~= 0 and tostring(count) or "", opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.lsp_progress(opts)
  return function()
    ...
    return nvim.status.utils.stylize(
      ...
      and string.format(
        " %%<%s %s %s (%s%%%%) ",
        nvim.get_icon("LSP" .. ((Lsp.percentage or 0) >= 70 and { "Loaded", "Loaded", "Loaded" } or {
            "Loading1",
            "Loading2",
            "Loading3",
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.lsp_client_names(opts)
  opts = nvim.default_tbl(opts, { expand_null_ls = true, truncate = 0.25 })
  return function(self)
    ...
    for _, client in pairs(vim.lsp.get_active_clients { bufnr = self and self.bufnr or 0 }) do
      if client.name == "null-ls" and opts.expand_null_ls then
        ...
        for _, type in ipairs { "FORMATTING", "DIAGNOSTICS" } do
          for _, source in ipairs(nvim.null_ls_sources(vim.bo.filetype, type)) do
    ...
    if type(opts.truncate) == "number" then
      local max_width = math.floor(nvim.status.utils.width() * opts.truncate)
      if #str > max_width then str = string.sub(str, 0, max_width) .. "…" end
    end
    return nvim.status.utils.stylize(str, opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.treesitter_status(opts)
  return function()
    return nvim.status.utils.stylize(require("nvim-treesitter.parser").has_parser() and "TS" or "", opts)
  end
end
...
-- @see astronvim.status.utils.stylize
function nvim.status.provider.str(opts)
  opts = nvim.default_tbl(opts, { str = " " })
  return nvim.status.utils.stylize(opts.str, opts)
end
...
function nvim.status.condition.is_active() return vim.api.nvim_get_current_win() == tonumber(vim.g.actual_curwin) end
...
function nvim.status.condition.buffer_matches(patterns, bufnr)
  for kind, pattern_list in pairs(patterns) do
    if nvim.status.env.buf_matchers[kind](pattern_list, bufnr) then return true end
  end
  return false
end
...
function nvim.status.condition.is_macro_recording() return vim.fn.reg_recording() ~= "" end
...
function nvim.status.condition.is_hlsearch() return vim.v.hlsearch ~= 0 end
...
function nvim.status.condition.is_git_repo(bufnr)
...
function nvim.status.condition.git_changed(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
function nvim.status.condition.file_modified(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
function nvim.status.condition.file_read_only(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
function nvim.status.condition.has_diagnostics(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
function nvim.status.condition.has_filetype(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
function nvim.status.condition.aerial_available() return package.loaded["aerial"] end
...
function nvim.status.condition.lsp_attached(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
function nvim.status.condition.treesitter_available(bufnr)
...
function nvim.status.utils.stylize(str, opts)
  opts = nvim.default_tbl(opts, {
    padding = { left = 0, right = 0 },
  ...
  local icon = nvim.pad_string(nvim.get_icon(opts.icon.kind), opts.icon.padding)
  return str
      and (str ~= "" or opts.show_empty)
      and opts.separator.left .. nvim.pad_string(icon .. str, opts.padding) .. opts.separator.right
      or ""
end
...
function nvim.status.component.fill(opts)
  return nvim.default_tbl(opts, { provider = nvim.status.provider.fill() })
end
...
function nvim.status.component.file_info(opts)
  opts = nvim.default_tbl(opts, {
    file_icon = {
      hl = nvim.status.hl.file_icon "statusline",
      padding = { left = 1, right = 1 },
    }, -- TODO: REWORK THIS
    filename = {},
    file_modified = { padding = { left = 1 } },
    file_read_only = { padding = { left = 1 } },
    surround = { separator = "left", color = "file_info_bg", condition = nvim.status.condition.has_filetype },
    hl = nvim.status.hl.get_attributes "file_info",
  })
  return nvim.status.component.builder(nvim.status.utils.setup_providers(opts, {
    "file_icon",
    "unique_path",
    "filename",
    "filetype",
...
function nvim.status.component.tabline_file_info(opts)
  return nvim.status.component.file_info(nvim.default_tbl(opts, {
    file_icon = {
      condition = function(self) return not self._show_picker end,
      hl = nvim.status.hl.file_icon "tabline",
    },
    unique_path = {
      hl = function(self) return nvim.status.hl.get_attributes(self.tab_type .. "_path") end,
    },
    close_button = {
      hl = function(self) return nvim.status.hl.get_attributes(self.tab_type .. "_close") end,
      padding = { left = 1, right = 1 },
      on_click = {
        callback = function(_, minwid) nvim.close_buf(minwid) end,
    ...
    hl = function(self)
      ...
      return nvim.status.hl.get_attributes(tab_type)
    end,
    surround = false,
  }))
end
...
function nvim.status.component.nav(opts)
  opts = nvim.default_tbl(opts, {
    ruler = {},
    percentage = { padding = { left = 1 } },
    scrollbar = { padding = { left = 1 }, hl = { fg = "scrollbar" } },
    surround = { separator = "right", color = "nav_bg" },
    hl = nvim.status.hl.get_attributes "nav",
    update = { "CursorMoved", "BufEnter" },
  })
  return nvim.status.component.builder(
    nvim.status.utils.setup_providers(opts, { "ruler", "percentage", "scrollbar" })
  )
end
...
function nvim.status.component.macro_recording(opts)
  opts = nvim.default_tbl(opts, {
    macro_recording = { icon = { kind = "MacroRecording", padding = { right = 1 } } },
    surround = {
      separator = "center",
      color = "macro_recording_bg",
      condition = nvim.status.condition.is_macro_recording,
    },
    hl = nvim.status.hl.get_attributes "macro_recording",
    update = { "RecordingEnter", "RecordingLeave" },
  })
  return nvim.status.component.builder(nvim.status.utils.setup_providers(opts, { "macro_recording" }))
end
...
function nvim.status.component.cmd_info(opts)
  opts = nvim.default_tbl(opts, {
    macro_recording = {
    ...
    search_count = {
      ...
      padding = { left = 1 },
      condition = nvim.status.condition.is_hlsearch,
    },
    surround = {
      separator = "center",
      color = "cmd_info_bg",
      condition = function()
        return nvim.status.condition.is_hlsearch() or nvim.status.condition.is_macro_recording()
      end,
    },
    condition = function() return vim.opt.cmdheight:get() == 0 end,
    hl = nvim.status.hl.get_attributes "cmd_info",
  })
  return nvim.status.component.builder(
    nvim.status.utils.setup_providers(opts, { "macro_recording", "search_count" })
  )
end
...
function nvim.status.component.mode(opts)
  opts = nvim.default_tbl(opts, {
    mode_text = false,
    paste = false,
    spell = false,
    surround = { separator = "left", color = nvim.status.hl.mode_bg },
    hl = nvim.status.hl.get_attributes "mode",
    update = "ModeChanged",
  })
  if not opts["mode_text"] then opts.str = { str = " " } end
  return nvim.status.component.builder(
    nvim.status.utils.setup_providers(opts, { "mode_text", "str", "paste", "spell" })
  )
end
...
function nvim.status.component.breadcrumbs(opts)
  opts = nvim.default_tbl(
    opts,
    { padding = { left = 1 }, condition = nvim.status.condition.aerial_available, update = "CursorMoved" }
  )
  opts.init = nvim.status.init.breadcrumbs(opts)
  return opts
end
...
function nvim.status.component.git_branch(opts)
  opts = nvim.default_tbl(opts, {
    git_branch = { icon = { kind = "GitBranch", padding = { right = 1 } } },
    surround = { separator = "left", color = "git_branch_bg", condition = nvim.status.condition.is_git_repo },
    hl = nvim.status.hl.get_attributes "git_branch",
    on_click = {
      name = "heirline_branch",
      callback = function()
        if nvim.is_available "telescope.nvim" then
          vim.defer_fn(function() require("telescope.builtin").git_branches() end, 100)
        end
      end,
    },
    update = { "User", pattern = "GitSignsUpdate" },
    init = nvim.status.init.update_events { "BufEnter" },
  })
  return nvim.status.component.builder(nvim.status.utils.setup_providers(opts, { "git_branch" }))
end
...
function nvim.status.component.git_diff(opts)
  opts = nvim.default_tbl(opts, {
    ...
    hl = nvim.status.hl.get_attributes "git_diff",
    on_click = {
      name = "heirline_git",
      callback = function()
        if nvim.is_available "telescope.nvim" then
    ...
    surround = { separator = "left", color = "git_diff_bg", condition = nvim.status.condition.git_changed },
    update = { "User", pattern = "GitSignsUpdate" },
    init = nvim.status.init.update_events { "BufEnter" },
  })
  return nvim.status.component.builder(
    nvim.status.utils.setup_providers(opts, { "added", "changed", "removed" }, function(p_opts, provider)
      local out = nvim.status.utils.build_provider(p_opts, provider)
      if out then
...
function nvim.status.component.diagnostics(opts)
  opts = nvim.default_tbl(opts, {
    ...
    surround = { separator = "left", color = "diagnostics_bg", condition = nvim.status.condition.has_diagnostics },
    hl = nvim.status.hl.get_attributes "diagnostics",
    on_click = {
  ...
  return nvim.status.component.builder(
    nvim.status.utils.setup_providers(opts, { "ERROR", "WARN", "INFO", "HINT" }, function(p_opts, provider)
      local out = nvim.status.utils.build_provider(p_opts, provider)
      if out then
...
function nvim.status.component.treesitter(opts)
  opts = nvim.default_tbl(opts, {
    str = { str = "TS", icon = { kind = "ActiveTS" } },
    surround = {
      separator = "right",
      color = "treesitter_bg",
      condition = nvim.status.condition.treesitter_available,
    },
    hl = nvim.status.hl.get_attributes "treesitter",
    update = { "OptionSet", pattern = "syntax" },
    init = nvim.status.init.update_events { "BufEnter" },
  })
  return nvim.status.component.builder(nvim.status.utils.setup_providers(opts, { "str" }))
end
...
function nvim.status.component.lsp(opts)
  opts = nvim.default_tbl(opts, {
    lsp_progress = {
      str = "",
      padding = { right = 1 },
    ...
    hl = nvim.status.hl.get_attributes "lsp",
    surround = { separator = "right", color = "lsp_bg", condition = nvim.status.condition.lsp_attached },
    on_click = {
      name = "heirline_lsp",
      callback = function()
  ...
  return nvim.status.component.builder(
    nvim.status.utils.setup_providers(
      opts,
      ...
      function(p_opts, provider, i)
        return p_opts
            and {
              flexible = i,
              nvim.status.utils.build_provider(p_opts, nvim.status.provider[provider](p_opts)),
              nvim.status.utils.build_provider(p_opts, nvim.status.provider.str(p_opts)),
            }
            or false
      end
    )
...
function nvim.status.component.builder(opts)
  opts = nvim.default_tbl(opts, { padding = { left = 0, right = 0 } })
  local children = {}
  if opts.padding.left > 0 then -- add left padding
    table.insert(children, { provider = nvim.pad_string(" ", { left = opts.padding.left - 1 }) })
  end
  for key, entry in pairs(opts) do
    if
        type(key) == "number"
        and type(entry) == "table"
        and nvim.status.provider[entry.provider]
        and (entry.opts == nil or type(entry.opts) == "table")
    then
      entry.provider = nvim.status.provider[entry.provider](entry.opts)
    end
    children[key] = entry
  end
  if opts.padding.right > 0 then -- add right padding
  ...
  return opts.surround
      and nvim.status.utils.surround(
...
function nvim.status.utils.build_provider(opts, provider, _)
  return opts
      and {
        provider = provider,
        opts = opts,
...
function nvim.status.utils.setup_providers(opts, providers, setup)
  setup = setup or nvim.status.utils.build_provider
...
function nvim.status.utils.width(is_winbar)
...
function nvim.status.utils.surround(separator, color, component, condition)
  ...
  separator = type(separator) == "string" and nvim.status.env.separators[separator] or separator
...
function nvim.status.utils.is_valid_buffer(bufnr) -- TODO v3: remove this function
...
function nvim.status.utils.get_valid_buffers() -- TODO v3: remove this function
  return vim.tbl_filter(nvim.status.utils.is_valid_buffer, vim.api.nvim_list_bufs())
end
...
function nvim.status.utils.encode_pos(line, col, winnr)
...
function nvim.status.utils.decode_pos(c)
