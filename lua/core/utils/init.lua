--- ### nvim Utilities
--
-- This module is automatically loaded by nvim on during it's initialization into global variable `nvim`
--
-- This module can also be manually loaded with `local nvim = require "core.utils"`
--
-- @module core.utils
-- @copyright 2022
...
local function func_or_extend(overrides, default, extend)
  ...
  if extend then
    ...
    if type(overrides) == "table" then
      default = nvim.default_tbl(overrides, default)
...
function nvim.default_tbl(opts, default)
  opts = opts or {}
...
function nvim.get_hlgroup(name, fallback)
...
function nvim.initialize_icons()
  nvim.icons = require "core.icons.nerd_font"
  nvim.text_icons = require "core.icons.text"
end

--- Get an icon from `lspkind` if it is available and return it
-- @param kind the kind of icon in `lspkind` to retrieve
-- @return the icon
function nvim.get_icon(kind)
  local icon_pack = vim.g.icons_enabled and "icons" or "text_icons"
  if not nvim[icon_pack] then nvim.initialize_icons() end
  return nvim[icon_pack] and nvim[icon_pack][kind] or ""
end
...
function nvim.notify(msg, type, opts)
  vim.schedule(function() vim.notify(msg, type, nvim.default_tbl(opts, { title = "nvim" })) end)
end
...
function nvim.initialize_packer()
  ...
  if packer_avail then
    ...
    local run_me, _ = loadfile(
      ({ compile_path = nvim.default_compile_path }).compile_path
    )
    if run_me then
      ...
      vim.api.nvim_create_autocmd("User", {
        ...
        callback = function()
          ...
          nvim.notify "Mason is installing packages if configured, check status with :Mason"
        end,
      })
      vim.opt.cmdheight = 1
...
function nvim.lazy_load_commands(plugin, commands)
  if type(commands) == "string" then commands = { commands } end
  if nvim.is_available(plugin) and not packer_plugins[plugin].loaded then
...
function nvim.system_open(path)
  ...
  elseif vim.fn.has "unix" == 1 then
    ...
    nvim.notify("System open is not supported on this OS!", "error")
  end
end
...
function nvim.toggle_term_cmd(opts)
  local terms = nvim.user_terminals
  ...
  -- toggle the terminal
  nvim.user_terminals[opts.cmd][num]:toggle()
end
...
function nvim.load_plugin_with_func(plugin, module, func_names)
...
function nvim.is_valid_buffer(bufnr)
...
function nvim.close_buf(bufnr, force)
