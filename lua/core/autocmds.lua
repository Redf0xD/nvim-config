...
if vim.g.heirline_bufferline then
  ...
  cmd({ "BufAdd", "BufEnter" }, {
    ...
    callback = function(args)
      ...
      vim.t.bufs = vim.tbl_filter(nvim.is_valid_buffer, vim.t.bufs)
    end,
  })
  cmd("BufDelete", {
...
cmd({ "VimEnter", "FileType", "BufEnter", "WinEnter" }, {
  ...
  pattern = "*",
  callback = function() nvim.set_url_match() end,
})
...
vim.api.nvim_create_autocmd("BufRead", {
  ...
  callback = function()
    ...
    if vim.v.shell_error == 0 then
      ...
      vim.tbl_map(function(plugin) packer.loader(plugin) end, nvim.git_plugins)
    end
  end,
})
vim.api.nvim_create_autocmd({ "BufRead", "BufWinEnter", "BufNewFile" }, {
  ...
  callback = function(args)
    if not (vim.fn.expand "%" == "" or vim.api.nvim_buf_get_option(args.buf, "buftype") == "nofile") then
      ...
      local packer = require "packer"
      vim.tbl_map(function(plugin) packer.loader(plugin) end, nvim.file_plugins)
    end
  end,
})
