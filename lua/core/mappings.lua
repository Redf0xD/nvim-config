...
maps.n["gx"] = { function() nvim.system_open() end, desc = "Open the file under cursor with system app" }
...
if vim.g.heirline_bufferline then
  -- Manage Buffers
  maps.n["<leader>c"] = { function() nvim.close_buf(0) end, desc = "Close buffer" }
  maps.n["<leader>C"] = { function() nvim.close_buf(0, true) end, desc = "Force close buffer" }
  maps.n["<S-l>"] = { function() nvim.nav_buf(vim.v.count > 0 and vim.v.count or 1) end, desc = "Next buffer" }
  maps.n["<S-h>"] =
  { function() nvim.nav_buf( -(vim.v.count > 0 and vim.v.count or 1)) end, desc = "Previous buffer" }
  maps.n[">b"] =
  { function() nvim.move_buf(vim.v.count > 0 and vim.v.count or 1) end, desc = "Move buffer tab right" }
  maps.n["<b"] =
  { function() nvim.move_buf( -(vim.v.count > 0 and vim.v.count or 1)) end, desc = "Move buffer tab left" }

  maps.n["<leader>bb"] = {
    function()
      nvim.status.heirline.buffer_picker(function(bufnr) vim.api.nvim_win_set_buf(0, bufnr) end)
    end,
    desc = "Select buffer from tabline",
  }
  maps.n["<leader>bd"] = {
    function()
      nvim.status.heirline.buffer_picker(function(bufnr) nvim.close_buf(bufnr) end)
    end,
    desc = "Delete buffer from tabline",
  }
  maps.n["<leader>b\\"] = {
    function()
      nvim.status.heirline.buffer_picker(function(bufnr)
        vim.cmd.split()
  ...
  -- HOP
  maps.n["<leader>h"] = { ":HopWord<CR>", desc = "HopWord" }
...
if is_available "telescope.nvim" then
  ...
  if nvim.is_available "nvim-notify" then
    maps.n["<leader>sn"] =
...
if is_available "toggleterm.nvim" then
  local toggle_term_cmd = nvim.toggle_term_cmd
...
if is_available "nvim-autopairs" then
  maps.n["<leader>ua"] = { function() nvim.ui.toggle_autopairs() end, desc = "Toggle autopairs" }
end
maps.n["<leader>ub"] = { function() nvim.ui.toggle_background() end, desc = "Toggle background" }
if is_available "nvim-cmp" then
  maps.n["<leader>uc"] = { function() nvim.ui.toggle_cmp() end, desc = "Toggle autocompletion" }
end
if is_available "nvim-colorizer.lua" then
  maps.n["<leader>uC"] = { "<cmd>ColorizerToggle<cr>", desc = "Toggle color highlight" }
end
maps.n["<leader>uS"] = { function() nvim.ui.toggle_conceal() end, desc = "Toggle conceal" }
maps.n["<leader>ud"] = { function() nvim.ui.toggle_diagnostics() end, desc = "Toggle diagnostics" }
maps.n["<leader>ug"] = { function() nvim.ui.toggle_signcolumn() end, desc = "Toggle signcolumn" }
maps.n["<leader>ui"] = { function() nvim.ui.set_indent() end, desc = "Change indent setting" }
maps.n["<leader>ul"] = { function() nvim.ui.toggle_statusline() end, desc = "Toggle statusline" }
maps.n["<leader>un"] = { function() nvim.ui.change_number() end, desc = "Change line numbering" }
maps.n["<leader>us"] = { function() nvim.ui.toggle_spell() end, desc = "Toggle spellcheck" }
maps.n["<leader>up"] = { function() nvim.ui.toggle_paste() end, desc = "Toggle paste mode" }
maps.n["<leader>ut"] = { function() nvim.ui.toggle_tabline() end, desc = "Toggle tabline" }
maps.n["<leader>uu"] = { function() nvim.ui.toggle_url_match() end, desc = "Toggle URL highlight" }
maps.n["<leader>uw"] = { function() nvim.ui.toggle_wrap() end, desc = "Toggle wrap" }
maps.n["<leader>uy"] = { function() nvim.ui.toggle_syntax() end, desc = "Toggle syntax highlight" }
maps.n["<leader>uN"] = { function() nvim.ui.toggle_ui_notifications() end, desc = "Toggle UI notifications" }

nvim.set_mappings(maps)
